...
class PSO(object):
  ...
  def __init__(self, obj_func, box_bounds, n_parts=5, topology="gbest",
               inertia_start=0.9, inertia_end=0.4, nostalgia=2.1, societal=2.1):
    ...
    self.pos = np.random.uniform(self.lower_bounds, self.upper_bounds, (self._n_parts, self._n_dims))
    ...
    self.vel = np.random.uniform(- self.vel_max, self.vel_max, (self._n_parts, self._n_dims))

    # Find the performance per particle
    # (updates self.perf, a matrix of self._n_parts rows and
    # self._n_dims cols)
    self.perf = np.array(list(map(self.obj_func_vectorized, self.pos))) # self.obj_func_vectorized(*self.pos.T)
  ...
  def _cache_neighbourhoods(self, topology):
    ...
    if self.topology == "gbest":
      ...
      if not n_sqrt**2 == n:
        raise Exception("Number of particles needs to be perfect square " +
  ...
  def _velocity_updates(self, itr, max_itr, use_constr_factor=True):
    ...
    """
    w_inertia = self.inertia_start + (self.inertia_end - self.inertia_start) * (itr / float(max_itr))

    inertia_vel_comp = w_inertia * self.vel
    nostalgia_vel_comp = self.nostalgia * np.random.rand() * (self.pbest - self.pos)
    societal_vel_comp = self.societal * np.random.rand() * (self.best_neigh - self.pos)
    ...
    if use_constr_factor:
      ...
      self.vel *= 2. / np.abs(2. - phi - np.sqrt((phi**2) - (4. * phi)))
  ...
  def _box_bounds_checking(self):
    ...
    self.vel[too_low | too_high] *= -np.random.random((self.vel[too_low | too_high]).shape)
  ...
  def _tstep(self, itr, max_itr):
    ...
    if self.topology == "gbest":
      ...
      # topologies
      best_neigh_idx = np.choose(self.perf[self.neighbourhoods].argmin(axis=1), self.neighbourhoods.T)
  ...
  def run(self, max_itr=100, tol_thres=None, tol_win=5):
